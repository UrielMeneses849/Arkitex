...
export default function TipoDeRegistro() {
    return (
        <Grid item xs={1} px={{ xs: 0, md: 5, lg: 15, xl: 20 }} mt={{ xs: '1rem', md: '0rem' }} >
            {/* */}
            <Grid container style={{ backgroundColor: "#FF9500"  }} borderRadius={"30px"}
                ...
                <Grid columns={2} display={"flex"} flexDirection={{ xs: 'column', md: 'row' }} sx={{gap:'1rem', margin: {xs:'0px',xl:"25px auto"}}}>
                ...
                <Grid columns={2} display={"flex"} flexDirection={{ xs: 'column', md: 'row' }} sx={{gap:'1rem'}}>
                    ...
                    <Grid className="textosEmpl"item xs={1} display={"flex"} gap={"1rem"} flexDirection={"column"} alignItems={{ xs: "center", md: 'start' }} >
